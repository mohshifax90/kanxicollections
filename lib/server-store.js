...
export async function loadStoreRow() {
  const rows = await supabaseFetch("GET", `?id=eq.${encodeURIComponent(FULL_ROW_ID)}&select=data&limit=1`);
  return rows?.[0]?.data || { users: [], orders: [], payments: [], batches: [], paymentSettings: {}, deliverySettings: {}, orderSeq: 1 };
}

export async function saveStoreRow(data) {
  const storefront = buildStorefrontSnapshot(data);
  const updatedAt = new Date().toISOString();
  await Promise.all([
    supabaseFetch("POST", "", { id: FULL_ROW_ID, data, updated_at: updatedAt }),
    supabaseFetch("POST", "", { id: STOREFRONT_ROW_ID, data: storefront, updated_at: updatedAt }),
  ]);
}
