...
function publicMediaSource(source, kind, params = {}) {
  const value = String(source || "").trim();
  if (!value) return "";
  if (/^https?:\/\//i.test(value) || value.startsWith("/")) return value;
  return mediaRef(kind, params);
}
...
function buildHomepageSnapshot(homepage = {}) {
  ...
  return {
    header: {
      logo: publicMediaSource(homepage.header?.logo, "header-logo"),
    },
    collections: {
      ...
      cards: collections.map((card) => ({
        ...
        image: publicMediaSource(card.image, "home-card", { id: card.id }),
...
export function buildStorefrontSnapshot(data = {}) {
  ...
  return {
    ...
    categories: categories.map((category) => ({
      ...
      slug: category.slug,
      icon: category.icon || "",
      image: publicMediaSource(category.image, "category", { id: category.id }),
      ...
      brands: Array.isArray(category.brands)
        ? category.brands.map((brand) => ({
            id: brand.id,
            name: brand.name || "",
            logo: publicMediaSource(brand.logo, "brand", { categoryId: category.id, id: brand.id }),
          }))
        : [],
    })),
    subcategories: subcategories.map((subcategory) => ({
      ...
      image: publicMediaSource(subcategory.image, "subcategory", { id: subcategory.id }),
    })),
    tags: tags.map((tag) => ({
      id: tag.id,
      name: tag.name,
    ...
    products: products.map((product) => ({
      ...
      image:
        product.image || (Array.isArray(product.images) ? product.images[0] : "") || product.photo
          ? publicMediaSource(product.image || (Array.isArray(product.images) ? product.images[0] : "") || product.photo, "product", { id: product.id })
          : "",
