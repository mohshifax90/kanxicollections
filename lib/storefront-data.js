...
import { readCacheJson, writeCacheJson } from "@/lib/upstash-cache";
...
const SUPABASE_TIMEOUT_MS = IS_DEV ? 8_000 : 5_000;
const UPSTASH_ROUTE_TTL_SECONDS = 60;
const UPSTASH_FULL_TTL_SECONDS = 45;
...
async function fetchRouteRow(rowId) {
  ...
  const promise = (async () => {
    try {
      const redisCached = await readCacheJson("route", rowId);
      if (redisCached) {
        routeRowCache.set(rowId, { data: redisCached, at: Date.now() });
        return redisCached;
      }
      const data = await supabaseFetchRow(rowId, !IS_DEV);
      if (data) {
        routeRowCache.set(rowId, { data, at: Date.now() });
        await writeCacheJson("route", rowId, data, UPSTASH_ROUTE_TTL_SECONDS);
      }
      return data;
    } catch {
...
async function fetchFullStoreData() {
  ...
  inflightFullStorePromise = (async () => {
    try {
      const redisCached = await readCacheJson("full", FULL_ROW_ID);
      if (redisCached) {
        lastSuccessfulFullStore = redisCached;
        lastSuccessfulFullStoreAt = Date.now();
        return redisCached;
      }
      const data = await supabaseFetchRow(FULL_ROW_ID, !IS_DEV);
      if (data) {
        lastSuccessfulFullStore = data;
        lastSuccessfulFullStoreAt = Date.now();
        await writeCacheJson("full", FULL_ROW_ID, data, UPSTASH_FULL_TTL_SECONDS);
      }
...
async function persistRouteDatasets(data) {
  ...
  await Promise.all(
    rows.map((row) => writeCacheJson("route", row.id, row.data, UPSTASH_ROUTE_TTL_SECONDS)),
  );
  await writeCacheJson("full", FULL_ROW_ID, data, UPSTASH_FULL_TTL_SECONDS);
}
...
export async function fetchRawStoreData() {
  ...
  inflightStorePromise = (async () => {
    try {
      ...
      let data = null;
      try {
        data = await supabaseFetchRow(STOREFRONT_ROW_ID, !IS_DEV);
      } catch (_error) {
        data = null;
      }
      ...
      if (Array.isArray(finalData.products) && finalData.products.length) {
        ...
        lastSuccessfulStoreAt = Date.now();
        await writeCacheJson("route", STOREFRONT_ROW_ID, finalData, UPSTASH_ROUTE_TTL_SECONDS);
      }
      return finalData;
    } catch (error) {
...
export function primeStorefrontCaches(fullData = null, datasets = null) {
  ...
  if (datasets?.media) routeRowCache.set(MEDIA_ROW_ID, { data: datasets.media, at: now });
  ...
  const derived = {
    ...
    homepage: {
      ...
      hero: {
        ...
        caption: heroSource.caption || "",
        title: heroSource.caption || "",
...
function mediaDataFromFullData(fullData = {}) {
  const homepage = fullData.homepage || {};
  const categories = Array.isArray(fullData.categories) ? fullData.categories : [];
  const subcategories = Array.isArray(fullData.subcategories) ? fullData.subcategories : [];
  const products = Array.isArray(fullData.products) ? fullData.products : [];
  const collectionCards = Array.isArray(homepage.collections?.cards) ? homepage.collections.cards : [];

  return {
    headerLogo: cleanImage(homepage.header?.logo) || "",
    homeCards: Object.fromEntries(
      collectionCards.map((card) => [card.id, cleanImage(card.image) || ""]).filter(([id, image]) => id && image),
    ),
    categories: Object.fromEntries(
      categories.map((category) => [category.id, cleanImage(category.image) || ""]).filter(([id, image]) => id && image),
    ),
    subcategories: Object.fromEntries(
      subcategories
        .map((subcategory) => [subcategory.id, cleanImage(subcategory.image) || ""])
    ...
    brands: Object.fromEntries(
      categories.map((category) => [
        category.id,
        Object.fromEntries(
          (Array.isArray(category.brands) ? category.brands : [])
            .map((brand) => [brand.id, cleanImage(brand.logo) || ""])
            .filter(([id, image]) => id && image),
        ),
      ]),
    ),
    products: Object.fromEntries(
      products
        .map((product) => [
          product.id,
          cleanImage(product.image) ||
            cleanImage((Array.isArray(product.images) ? product.images[0] : "") || "") ||
            cleanImage(product.photo) ||
            "",
        ])
        .filter(([id, image]) => id && image),
...
export function buildRouteDatasets(fullData = {}) {
  ...
  const media = mediaDataFromFullData(fullData);
