...
async function fetchFullStoreData() {
  ...
  inflightFullStorePromise = (async () => {
    try {
      const data = await supabaseFetchRow(FULL_ROW_ID, true);
      if (data) {
...
export async function fetchRawStoreData() {
  ...
  inflightStorePromise = (async () => {
    try {
      ...
      let data = null;
      try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/${TABLE}?id=eq.${encodeURIComponent(STOREFRONT_ROW_ID)}&select=data&limit=1`, {
          headers: {
            apikey: SUPABASE_KEY,
            Authorization: `Bearer ${SUPABASE_KEY}`,
          },
      ...
      if (!data || snapshotTooLarge(data)) {
        const full = await supabaseFetchRow(FULL_ROW_ID, true);
        if (full) {
...
function buildDerivedStore(data, options = {}) {
  ...
  const presentSourceTypes = new Set(baseSections.map((section) => section.sourceType));
  const curatedSections = [
    { id: "flash-sale", title: "Flash Sale", sourceType: "offers", href: "/category" },
    { id: "new-arrivals", title: "New Arrivals", sourceType: "new_arrivals", href: "/category" },
    { id: "trending-today", title: "Trending Today", sourceType: "trending", href: "/category" },
    { id: "recommended", title: "Recommended For You", sourceType: "recommended", href: "/category" },
  ]
    .filter((section) => !presentSourceTypes.has(section.sourceType))
    .map((section) => ({
      ...section,
      products: findProductsBySource(data, section)
        .slice(0, 6)
        .map((product) => productSummaryMap.get(product.id) || productSummary(product, data, tags, categoriesById, subcategoriesById)),
    }))
    .filter((section) => section.products.length);

  const featuredSections = [...baseSections, ...curatedSections];
...
export async function getProductDetail(id) {
  ...
  const detailDerived = buildDerivedStore(source, { includeProductDetails: true });
  const product = detailDerived.productMap.get(id) || detailDerived.products[0] || null;
